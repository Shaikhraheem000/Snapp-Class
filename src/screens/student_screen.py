...
from src.database.db import get_all_students,create_student,get_student_subjects,get_student_attendance,unenroll_subject_student
    ...
    for i,sub_node in enumerate(subjects):
        ...
        def  unenroll_button(sid=sid, subject_name=sub['name']):
               if st.button('Unenroll from this course',type='tertiary',width='stretch',icon=':material/delete_forever:',key=f'unenroll_{sid}'):
                   unenroll_subject_student(student_id,sid) 
...
def student_screen():
    ...
    if show_registration:
           with st.container(border=True):
               ...
               new_name = st.text_input('Enter your name',placeholder='E.g Shaikh Raheem')
               ...
               if st.button("Create Account",type='primary'):
                   if new_name:
                       with st.spinner('creating profile...'):
                           ...
                           if embedding:
                               face_emd = embedding[0].tolist()
                               voice_emb =None
                               if audio_data:
                                   voice_emb =get_voice_embeddings(audio_data.read())
                               response_data = create_student(new_name,face_embedding=face_emd,voice_embedding=voice_emb)
