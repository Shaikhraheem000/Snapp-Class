...
def teacher_tab_take_attendance():
    ...
    if st.session_state.attendance_images:
        ...
        gallerey_cols = st.columns(4)

        for idx,img in enumerate(st.session_state.attendance_images):
            with gallerey_cols[idx % 4]:
...
def teacher_screen_login():
    ...
    st.space()
    st.space()
    teacher_username = st.text_input("Enter username",placeholder='example020')
    ...
    with btn1:
        if st.button("Login",type='primary',icon=':material/passkey:',width='stretch',shortcut='control+enter'):
    ...
    with btn2:
        if st.button("Register",type='secondary',icon=':material/passkey:',width='stretch',shortcut='control+enter'):
...
def teacher_screen_register():
    ...
    st.space()
    st.space()
    teacher_username = st.text_input("Enter Userid",placeholder='example@020')
    teacher_name = st.text_input("Enter username",placeholder='FullName')
    teacher_password = st.text_input("Enter password",type='password',placeholder='Enter password')
    ...
    with btn1:
        if st.button("Register Now",icon=':material/passkey:',width='stretch',shortcut='control+enter'):
            ...
            if success:
                ...
                st.error(message)
    with btn2:
        if st.button("Login Instead",type='primary',icon=':material/passkey:',width='stretch',shortcut='control+enter'):
            st.session_state.teacher_login_type = 'login'
    footer_dashboard()
